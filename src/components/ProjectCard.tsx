...
const estadoBadge: Record<string, { bg: string; text: string; border: string }> = {
  DISPONIBLE: { bg: 'rgba(184,142,58,0.12)', text: '#B88E3A', border: 'rgba(184,142,58,0.35)' },
...
export default function ProjectCard({ listing, navigate, waConfig }: Props) {
  const waUrl = waConfig
    ? buildWAUrl(waConfig, 'projectMsg', listing.titulo)
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden" style={{ height: '260px', backgroundColor: '#1a2e42' }}>
        {listing.imagen && (
          <img
            src={`${listing.imagen}&w=600&h=520&fit=crop&auto=format`}
            alt={listing.titulo}
            className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
          />
        )}
        <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(13,27,42,0.7) 0%, transparent 60%)' }} />
      ...
      <div className="flex flex-col flex-1 p-6" style={{ borderBottom: '1px solid rgba(245,242,236,0.08)' }}>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5" style={{ borderTop: '1px solid rgba(245,242,236,0.08)', paddingTop: '16px' }}>
          <div>
            ...
            <div style={{ fontFamily: "'Playfair Display'", fontSize: '15px', color: '#F5F2EC', fontWeight: 600 }}>{precio ?? '—'}</div>
          </div>
          {listing.fecha_entrega && (
            <div>
              <div style={{ fontFamily: "'Montserrat'", fontSize: '8px', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(245,242,236,0.35)', marginBottom: '3px' }}>Entrega</div>
